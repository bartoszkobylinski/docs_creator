...
function showResults() {
    ...
    // Check UML support and show panel if data is available
    checkUMLSupport();
}
...
// Show UML panel when data is loaded
function checkUMLSupport() {
    ...
    if (currentData && currentData.items && currentData.items.length > 0) {
        ...
        umlPanel.style.display = 'none';
    }
}

// Generate UML diagram
window.generateUMLDiagram = async function() {
    if (!currentData || !currentData.items) {
        alert('No data available for UML generation');
        return;
    }
    
    const configSelect = document.getElementById('uml-config');
    const selectedConfig = configSelect.value;
    
    try {
        const response = await fetch(`${apiBaseUrl}/api/uml/generate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        alert(`Failed to generate UML diagram: ${error.message}`);
    }
};

// Show custom UML configuration modal
window.showCustomUMLConfig = function() {
    document.getElementById('custom-uml-modal').style.display = 'flex';
};

// Close custom UML modal
window.closeCustomUMLModal = function() {
    document.getElementById('custom-uml-modal').style.display = 'none';
};

// Generate custom UML diagram
window.generateCustomUML = async function() {
    if (!currentData || !currentData.items) {
        alert('No data available for UML generation');
        return;
    }
    ...
    const customConfig = {
        include_methods: document.getElementById('include-methods').checked,
        include_attributes: document.getElementById('include-attributes').checked,
        include_private: document.getElementById('include-private').checked,
        show_stereotypes: document.getElementById('show-stereotypes').checked,
        color_by_type: document.getElementById('color-by-type').checked,
        group_by_package: document.getElementById('group-by-package').checked,
        max_classes: parseInt(document.getElementById('max-classes').value),
        layout_direction: document.getElementById('layout-direction').value
    };
    
    try {
        const response = await fetch(`${apiBaseUrl}/api/uml/generate-custom`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        console.error('Custom UML generation error:', error);
        alert(`Failed to generate custom UML diagram: ${error.message}`);
    }
};
...
function displayUMLResults(result) {
    ...
    if (result.success) {
        ...
        if (result.additional_diagrams) {
            Object.entries(result.additional_diagrams).forEach(([type, diagramData]) => {
                if (diagramData.url) {
                    const additionalDiagram = document.createElement('div');
                    additionalDiagram.className = 'diagram-item';
                    additionalDiagram.innerHTML = `
                        <h5>${type.charAt(0).toUpperCase() + type.slice(1)} Diagram</h5>
                        <img src="${diagramData.url}" alt="${type} UML Diagram" style="max-width: 100%; border: 1px solid #ddd; margin: 10px 0;">
                    `;
                    additionalContainer.appendChild(additionalDiagram);
                }
            });
        }
        ...
        if (result.analysis) {
            ...
            summary.className = 'uml-summary';
            summary.innerHTML = `
                <h5>Analysis Summary</h5>
                <ul>
                    <li>Classes found: ${result.analysis.classes_found}</li>
                    <li>Relationships found: ${result.analysis.relationships_found}</li>
                    <li>Packages: ${result.analysis.packages.join(', ')}</li>
                </ul>
            `;
