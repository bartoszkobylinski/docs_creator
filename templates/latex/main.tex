...
\pagestyle{fancy}
\fancyhf{}
\rhead{\VAR{latex_escape(project_name)}}
\lhead{Documentation}
\rfoot{Page \thepage}
...
\hypersetup{
    ...
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={\VAR{latex_escape(project_name)} Documentation},
...
\title{\VAR{latex_escape(project_name)}\\Documentation}
...
\BLOCK{if business_overview}
\section{Business Overview}

\BLOCK{if business_overview.project_purpose}
\textbf{Project Purpose:} \VAR{latex_escape(business_overview.project_purpose)}

\BLOCK{endif}
\BLOCK{if business_overview.business_context}
\textbf{Business Context:} \VAR{latex_escape(business_overview.business_context)}

\BLOCK{endif}
\BLOCK{if business_overview.key_business_value}
\textbf{Key Business Value:} \VAR{latex_escape(business_overview.key_business_value)}
...
This document provides comprehensive technical documentation for \textbf{\VAR{latex_escape(project_name)}}. It includes API documentation, code analysis, and coverage statistics.
...
\BLOCK{for module in modules}
    \item \texttt{\VAR{latex_escape(module)}}
\BLOCK{endfor}
\end{itemize}
...
\subsubsection{\texttt{\VAR{latex_escape(item.qualname)}}}

\begin{itemize}
    \item \textbf{Module:} \texttt{\VAR{latex_escape(item.module)}}
    \item \textbf{File:} \texttt{\VAR{latex_escape(item.file_path) if item.file_path else 'N/A'}}
...
\begin{quote}
\VAR{latex_escape(item.docstring)}
\end{quote}
\BLOCK{else}
...
\begin{itemize}
    ...
    \item \textbf{Packages:} \VAR{latex_escape(', '.join(uml_diagrams.analysis.get('packages', [])))}
\end{itemize}
\BLOCK{endif}
